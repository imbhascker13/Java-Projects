...
public class KalingMusic {
	...
	@Autowired
	private KalingaMusicService kalingaMusicService;

	@PostMapping(value = "/insert")
	public AlbumDto insertdata(@RequestBody AlbumDto album) {

		kalingaMusicService.insertAlbum(album);
		return album;
	...
	public List<SongDto> getData(@PathVariable int songId) throws ServiceException {
		...
		List<SongDto> songs = kalingaMusicService.getAllSongs(songId);
		return songs;
	...
	public int getCount(@PathVariable int albumId) {
		...
		return kalingaMusicService.getCountOfSongsInAlbum(albumId);
